...
import { ActivityIndicator, TextInput, StyleSheet, FlatList, TouchableOpacity, View, Alert } from 'react-native';
...
export default class PotentialScreen extends React.Component {
    ...
    onSearch = async () => {
        try {
            const res = await this.rsaApi.getPotentials(this.state.token, `?search=${this.state.inputSearch}`);
            if(!res.code) {
    ...
    onEndReached = async () => {
        ...
        try {
            const res = await this.rsaApi.getPotentials(this.state.token, `?page=${this.state.page}`);
            if(!res.code) {
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.contentTitle}>
                    ...
                    >
                        <FontAwesomeIcon icon={faArrowLeft} name='arrow-left' style={styles.iconBack}/>
                        <TextCatam font='bold' others={styles.textTitle}>Clientes Potenciales</TextCatam>
                    </TouchableOpacity>
                    <TouchableOpacity
                        onPress={() => this.props.navigation.navigate('AddStep1')}
                        style={styles.btnAdd}
                    >
                        <FontAwesomeIcon icon={faAdd} name='ios-add-circle' style={{fontSize:30, color: Colors.greenLight}}/>
                    </TouchableOpacity>
                </View>
